...
private:
    ...
    static ftxui::Element render_view_help(View& view) {
        Help help { view.help() };
        ...
        auto child = view.active_child();
        if (child) {
            return ftxui::hbox({
                ftxui::hbox(shortcut_elements),
                render_view_help(*child)
            });
        } else {
    ...
    virtual ftxui::Component renderer() override {
        ...
        return ftxui::Renderer(container, [&] {
            return ftxui::vbox({
                ...
                render_view_help(*this) | ftxui::size(ftxui::HEIGHT, ftxui::GREATER_THAN, 1)
            });
        });
    }
};
