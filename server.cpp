...
asio::awaitable<void> do_session(beast::tcp_stream stream, std::shared_ptr<std::string const> doc_root) {
...
asio::awaitable<void> do_listen(asio::ip::tcp::endpoint endpoint, std::shared_ptr<std::string const> doc_root) {
...
int server_main(const char* address_str, unsigned short port, const char* doc_root_str, int threads) {
    ...
    std::cout << "serving " << doc_root_str << " on " << address_str << ":" << port << " (" << threads << " threads)" << std::endl;
