...
inline const std::array<ftxui::Event, keycode_to_integral(Keycode::MAX) + 1>
get_keycode_events() {
  return {
      ftxui::Event::Character('a'), ftxui::Event::Character('b'),
      ftxui::Event::Character('c'), ftxui::Event::Character('d'),
      ftxui::Event::Character('e'), ftxui::Event::Character('f'),
      ftxui::Event::Character('g'), ftxui::Event::Character('i'),
      ftxui::Event::Character('m'), ftxui::Event::Character('n'),
      ftxui::Event::Character('o'), ftxui::Event::Character('p'),
      ftxui::Event::Character('q'), ftxui::Event::Character('r'),
      ftxui::Event::Character('s'), ftxui::Event::Character('t'),
      ftxui::Event::Character('u'), ftxui::Event::Character('v'),
      ftxui::Event::Character('w'), ftxui::Event::Character('x'),
      ftxui::Event::Character('y'), ftxui::Event::Character('z'),
      ftxui::Event::Character(':'), ftxui::Event::Return,
  };
};
...
public:
  ...
  Command(std::string name,
      ...
      : name(std::move(name)), default_keycode(std::nullopt),
        execute(std::move(execute)) {}
  Command(std::string name, Keycode default_keycode,
          std::function<void(CommandRuntime &command_runtime)> execute)
      : name(std::move(name)), default_keycode(default_keycode),
        execute(std::move(execute)) {}
};
...
public:
  ...
  CommandSet(std::optional<std::string> title, std::vector<Command> commands)
...
public:
  ...
  virtual std::shared_ptr<View> active_child() { return nullptr; }
  virtual CommandSet commands() { return {std::nullopt, {}}; };
...
private:
  ...
  std::array<KeycodeMapEntry, keycode_to_integral(Keycode::MAX) + 1>
      keycode_map;
  static std::vector<CommandSet> init_command_set_elements(View &root_view) {
    ...
    for (auto view{root_view.active_child()}; view;
         view = view->active_child()) {
    ...
    for (const CommandSet &command_set : command_sets) {
      for (const Command &command : command_set.commands) {
        if (command.default_keycode) {
          keycode_map[keycode_to_integral(*command.default_keycode)] =
              KeycodeMapEntry{&command_set, &command};
        }
      }
    }
  }
...
public:
  enum class StatusLineVariant { INFO, ERROR_WARN, ERROR_FAIL };
  class StatusLine {
  public:
    const std::string message;
    const StatusLineVariant variant;
    StatusLine(std::string message, StatusLineVariant variant)
...
private:
  ...
  int tab_index = 0;
  ftxui::App &app;
  std::array<ftxui::Event, keycode_to_integral(Keycode::MAX) + 1>
      keycode_events;
  ...
  static ftxui::Element render_help(const MultiCommandSet &multi_command_set) {
    ...
    for (std::size_t i = 0; i < multi_command_set.command_sets.size(); i++) {
      ...
      if (command_set.title) {
        shortcut_elements.push_back(ftxui::text(*command_set.title + " ") |
                                    color(ftxui::Color::Cyan));
      }
      for (const Command &command : command_set.commands) {
        if (command.default_keycode &&
            multi_command_set.lookup_keypress(*command.default_keycode) ==
                &command) {
          shortcut_elements.push_back(
              ftxui::text(
                  std::string(keycode_display_text(*command.default_keycode)) +
                  " ") |
              ftxui::dim);
          shortcut_elements.push_back(ftxui::text(command.name + " ") |
...
public:
  explicit RootView(InitInteractive init_interactive)
      : app(init_interactive.app), executor(init_interactive.async_executor),
        timer(executor), command_runtime(*this),
        keycode_events(get_keycode_events()) {}
  ...
  virtual CommandSet commands() override {
    return {"App",
            {
                {"command", Keycode::RETURN,
                 [&](CommandRuntime &rt) { rt.info("command"); }},
                {"quit", Keycode::Q, [&](CommandRuntime &rt) { app.Exit(); }},
                {"refresh all", Keycode::R,
                 [&](CommandRuntime &rt) { renderer(); }},
            }};
  }
  virtual ftxui::Component renderer() override {
    ...
    tab_menu =
        Menu(&tab_entries, &tab_index, ftxui::MenuOption::HorizontalAnimated());
    ...
    auto renderer{ftxui::Renderer(container, [&] {
      ...
      return ftxui::vbox(
          {tab_menu->Render(), tab_content->Render() | ftxui::flex,
           ftxui::separator() | color(ftxui::Color::GrayDark),
           (toast_value ? render_toast(*toast_value)
                        : render_help(*multi_command_set)) |
